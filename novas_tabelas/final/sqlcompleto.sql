...
-- Política para professores criarem turmas em seus cursos
DROP POLICY IF EXISTS "Professors can create classes for their courses" ON public.classes;
CREATE POLICY "Professors can create classes for their courses" ON public.classes FOR INSERT WITH CHECK (
...
-- Políticas de RLS para enrollments
...
CREATE POLICY "Admins can manage enrollments" ON public.enrollments FOR ALL USING (is_admin(auth.uid())) WITH CHECK (is_admin(auth.uid()));
